...
func TestRunContextSearchReturnsFileResults(t *testing.T) {
	root := t.TempDir()
	mustWriteFile(t, filepath.Join(root, "package.json"), "{\n  \"name\": \"web\"\n}\n")
	mustWriteFile(t, filepath.Join(root, "apps", "admin", "src", "lib", "format.ts"), `export function formatCurrency(value: number): string {
  return value.toFixed(2)
...
`)
	...
	// The target file must appear via path query round-robin.
	foundFile := false
	for _, r := range results.Results {
		if r.Kind == "file" && strings.Contains(r.Path, "format.ts") {
			foundFile = true
		}
		if r.Kind == "import" || r.Kind == "code_item" {
			t.Fatalf("run-context returned removed kind %q (tree-sitter removed): %#v", r.Kind, r)
		}
	}
	if !foundFile {
