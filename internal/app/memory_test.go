...
func TestMemoryProposeBeforeInitPrintsGuidance(t *testing.T) {
	...
	if code != 0 {
		t.Fatalf("memory propose before init exited %d, stderr: %s", code, stderr.String())
	}
	if got := stdout.String(); !strings.Contains(got, "Pactum is not initialized. Run: pactum init") {
		t.Fatalf("memory propose before init output mismatch:\n%s", got)
	}
}
...
func TestMemoryArtifactsArePortable(t *testing.T) {
	...
	runMemoryCommand(t, app, "review", "add-finding", runID, "path should be sanitized from "+root, "--file", "internal/app/memory.go", "--line", "42")
	runMemoryCommand(t, app, "review", "resolve", runID, "f_001", "--note", "fixed in "+root)
...
func setupApprovedReviewedMemoryRun(t *testing.T, root string) (App, artifacts.Paths, string, contractRunPathSet) {
	...
	writeMemoryGateReportForTest(t, runPaths, runID)
	runMemoryCommand(t, app, "review", "prepare", runID)
	runMemoryCommand(t, app, "review", "add-finding", runID, "memory candidate should include resolved findings", "--file", "internal/app/memory.go", "--line", "12")
	runMemoryCommand(t, app, "review", "resolve", runID, "f_001", "--note", "covered by deterministic memory tests")
