...
func renderContractReviewFixerPrompt(contract draftContract, currentVersion string, findings []contractReviewFinding) string {
	...
	fmt.Fprintln(&b, "- List fields are whole-list replacements in contract revise: if you include a list field, include every item that should remain, including unchanged items.")
	fmt.Fprintln(&b, "- For acceptance_criteria, validation.commands, assumptions, scope.in, and scope.out, do NOT output only the changed or newly added entries.")
	fmt.Fprintln(&b, "- For scope changes, use contract.scope.in and contract.scope.out as nested arrays.")
	fmt.Fprintln(&b, "- Do NOT use top-level contract.scope_in or contract.scope_out fields; they are invalid.")
	...
	fmt.Fprintln(&b, `      "in": ["...all in-scope items to keep, including unchanged and updated entries..."],`)
	fmt.Fprintln(&b, `      "out": ["...all out-of-scope items to keep, including unchanged and updated entries..."]`)
	fmt.Fprintln(&b, `    },`)
	fmt.Fprintln(&b, `    "acceptance_criteria": ["...all criteria to keep, including unchanged and updated entries..."],`)
	fmt.Fprintln(&b, `    "validation": {"commands": ["...all validation commands to keep, including unchanged and updated entries..."]},`)
	fmt.Fprintln(&b, `    "assumptions": ["...all assumptions to keep, including unchanged and updated entries..."]`)
	fmt.Fprintln(&b, "  }")
	fmt.Fprintln(&b, "}")
	fmt.Fprintln(&b, "```")
	fmt.Fprintln(&b)
