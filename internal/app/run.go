...
// the hits, deduping by document ID so the same result resurfaced by multiple
// queries collapses to one entry. Earlier queries are more important: results
// are kept in query order, then result order within a query, capped at limit.
// This is deterministic first-pass retrieval, not semantic ranking.
