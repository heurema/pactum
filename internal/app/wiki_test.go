...
// TestWikiGeneratedArtifactsHaveNoV2Strings covers Part H #13: the in-place
// change must not introduce v2 schema/artifact names.
func TestWikiGeneratedArtifactsHaveNoV2Strings(t *testing.T) {
	_, paths := initVueFixture(t)
	...
	artifactPaths = append(artifactPaths, paths.RepoMap, paths.LLMS, paths.MapManifest)
	err := filepath.WalkDir(paths.WikiDir, func(path string, entry fs.DirEntry, walkErr error) error {
		if walkErr != nil {
			return walkErr
		}
	...
	for _, path := range artifactPaths {
		...
		for _, phrase := range forbidden {
			if strings.Contains(content, phrase) {
				t.Fatalf("generated artifact %s contains forbidden v2 string %q", path, phrase)
			}
		}
	}
}
