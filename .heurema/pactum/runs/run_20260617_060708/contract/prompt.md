...
- Approval: approved
- Contract hash: 8ca8aacb9ed97ae1f80bfc3fd17588f20f9921702a4f51551c4fd4f289254199
...
- The authoritative pre-refactor stdout JSON schema, summary artifact schema, and ledger event-name baseline are enumerated explicitly in the schema-preservation and ledger-events acceptance criteria; these enumerations govern over any runtime snapshot and any executor need not separately capture a pre-refactor baseline for the purposes of criteria (k) and (l).
- internal/loop.Run propagates the Step closure's error verbatim: when the Step closure returns a non-nil error, Run immediately executes `return Outcome{}, err` (internal/loop/loop.go:78-79) with the same error value, performing no wrapping of any kind. Named sentinel errors returned by the Step closure are therefore detectable at the call site via errors.Is or direct pointer equality without any modification to the engine. The sentinel-based early-exit mechanism is fully implementable under the frozen-engine constraint.
- Every created proposal is counted as either accepted or duplicate; there is no third outcome for a created proposal in the current proposal-deduplication pipeline. This invariant underlies the `proposals.Created == 0 ↔ (accepted == 0 AND duplicates == 0)` equivalence used in the Clean predicate and stated in acceptance criteria (a) and (b). Implementors must verify this invariant holds in the current review_loop.go before relying on it — if any third proposal outcome exists, the Clean predicate and equivalence expressions in those criteria would be incorrect and must be revised before implementation proceeds.
...
Items:
- mem_011 [fresh] score=73 — Stagger the cold start of same-model reviewer groups in the review panel fan-...
- mem_015 [fresh] score=69 — Extract a shared bounded-loop engine into a new internal/loop package and por...
- mem_002 [stale] score=67 — Normalize the CLI command grammar for agent-first use: every stage exposes a ...
  reason: missing file internal/app/agents_doctor.go
  reason: missing file internal/app/agents_doctor_test.go
- mem_005 [fresh] score=64 — Make the CLI announce legal moves so an agent never guesses the pipeline stat...
- mem_007 [fresh] score=60 — Fix three valid external review findings. (1) pactum export must preserve its...
