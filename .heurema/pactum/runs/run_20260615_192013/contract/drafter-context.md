...
Generated: 2026-06-13T09:02:36Z
...
- Indexed files: 156
- Ignored files/directories: 2261
- Detected languages: 6
- Code items (best-effort hints): 1755
...
- `internal/agents/executor_test.go`: `go_func` `agents.TestRunSubprocessFiresOnFirstOutput`
...
## Search results
{
  "query": "Add gofmt enforcement to the build gate. Update the Makefile so that 'make check' fails when any tracked Go file is not gofmt-formatted (add a gofmt -l step that errors on non-empty output), and apply gofmt -w to fix any existing formatting drift. Do not change unrelated gate steps.",
  "queries": [
    "gofmt-formatted",
    "non-empty",
    "gofmt",
