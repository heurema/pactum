...
- Drafter: codex
- Accepted by: manual
- Accepted at: 2026-06-16T19:26:08Z
...
## Acceptance criteria
- internal/loop.Run calls Step with 1-based consecutive RoundContext.Round values and stops without domain knowledge of reviewers, fixers, contracts, files, or ledger events.
...
- internal/loop.Run returns Outcome.Reason "stalemate" after Patience consecutive rounds where Clean is false and Progress is false when Patience > 0, and resets the stale streak on either Clean or Progress.
...
- contract review still reports the existing public terminal_reason values: loop settled maps to "resolved", loop stalemate maps to "stalemate", and loop max maps to "max_rounds".
- Contract review clean and stale streak fields in each round summary match existing semantics: blocking_findings == 0 increments clean_streak; blocking findings reset clean_streak; unchanged contract version after a fixer round increments unchanged_version_streak; changed contract version resets it.
...
- Existing contract review behaviours remain covered and passing for no reviewers, single reviewer, multiple reviewers, partial lens failure, clean convergence, max rounds, and stale-version stalemate.
- When configured contract reviewers run, events.jsonl contains exactly one contract_review_loop_started event before contract reviewer/fixer attempt events and exactly one contract_review_loop_finished event after the loop reaches a terminal reason, each with the run_id populated.
- The no-reviewers contract review path remains a no-op and does not invoke loop.Run, agents, fixer attempts, or contract review loop ledger events.
