...
## Review Decisions
- f_001 [high] resolved internal/app/plan_review.go:284: Plan review can exit clean after a reviewer still fails to emit the mandatory findings block on the corrective retry.
  Resolution: After corrective retry, if blocks2 is empty, runPlanReviewerWithRetry now returns an error instead of nil findings. Changed in internal/app/plan_review.go (~line 284).
- f_002 [high] resolved internal/app/plan_review.go:284: Plan review silently treats a reviewer response that still lacks the mandatory findings block after corrective retry as a clean review.
  Resolution: Same fix as f_001: the corrective-retry path now explicitly errors when the response still has no valid pactum.reviewer_findings.v1alpha1 block, preventing silent clean-exit.
- f_003 [high] resolved internal/app/plan_review.go:485: The plan-review prompt contradicts the contract by instructing no-issue reviewers not to include the mandatory findings block.
  Resolution: writePlanReviewerOutputFormat now says 'Always include exactly one findings block. Use an empty array when there are no issues.' instead of the old contradictory 'Do not include an empty findings block.' (internal/app/plan_review.go:485).
- f_004 [medium] resolved internal/app/plan_review.go:388: Plan-review findings do not enforce the promised lens enum before writing JSON output.
  Resolution: Added isValidPlanReviewLens helper and a skip in planFindingsFromBlocks for any finding whose lens is not in planReviewLenses. Invalid-lens findings are silently dropped before JSON output (internal/app/plan_review.go).
- f_005 [medium] resolved internal/app/plan_review_test.go:224: The no-findings plan-review test treats a missing structured findings block as a clean review, instead of requiring an empty pactum.reviewer_findings.v1alpha1 block.
  Resolution: TestPlanReviewHelperProcess now emits an empty pactum.reviewer_findings.v1alpha1 block (via planReviewerEmptyFindingsOutput) when PACTUM_PLAN_REVIEWER_EMIT_FINDINGS is unset; updated test comment accordingly. TestPlanReviewNoFindingsExitsZero still expects exit 0 — the reviewer now satisfies the mandatory block requirement with an empty findings array (internal/app/plan_review_test.go).
- f_006 [medium] resolved internal/app/config_test.go:245: There is no regression test that pipeline.plan_review.by accepts and runs multiple configured reviewer agents.
  Resolution: Added TestPlanReviewMultipleReviewers (plan_review_test.go) which configures helper1 and helper2, verifies per-reviewer prompt.txt and attempt-1.txt artifacts, and confirms findings.json contains entries from both agents. Also added plan_review to the accepted panel stages in TestReadConfigPipelineMultiAgentByValidation (config_test.go).
- f_007 [medium] resolved internal/app/plan_review.go:260: Plan review silently treats missing structured reviewer output as a clean review.
  Resolution: Removed the early-return guard 'if len(stdoutBytes1) == 0 { return []planReviewFinding{}, nil }' in runPlanReviewerWithRetry; empty stdout now falls through to the corrective retry path just like prose-only output (internal/app/plan_review.go:260).
- f_008 [medium] resolved internal/app/cli.go:79: The new plan-review CLI and pipeline stage are user-visible but are not documented in the user-facing workflow/config docs.
  Resolution: Updated docs/flow.md: added pactum plan review to the stages table Contract row, and added a full prose paragraph documenting the plan review command, pipeline.plan_review.by config, the five lenses, the mandatory findings block requirement, artifact layout, exit codes, and --json output.
- f_009 [medium] resolved internal/app/contract.go:917: The new VACUOUS_VALIDATION scoped-validation rule is not documented in the user-facing plan documentation.
  Resolution: Updated docs/flow.md Contract section to document the VACUOUS_VALIDATION rule: tasks with non-empty expected_files must have at least one validation command scoped to those files; global-only commands are rejected with VACUOUS_VALIDATION; tasks with empty expected_files are exempt.
- Proposal summary: pending=0 accepted=9 rejected=0

## Reusable Project Knowledge
- scope: in scope: Extend contract plan validation so any plan task with non-empty expected_files must have at least one validation command scoped to an expected file path or an enclosing path/package segment.
- scope: in scope: Reject unscoped per-task validation with an actionable VACUOUS_VALIDATION issue during both contract load and contract revise.
- scope: in scope: Add pipeline.plan_review configuration using the existing stageBy scalar-or-list parsing; absent or empty by means no automated plan review.
- scope: in scope: Add pactum plan review [run_id] [--json] as a single-pass plan DAG reviewer over contract.plan.tasks, distinct from the existing pactum review plan code-review dry-run command.
- scope: in scope: Persist plan-review prompts, attempts, results, and structured findings under a run-local plan-review artifact area.
- scope: in scope: Use the existing reviewer_findings structured-output capture behavior, including parse-miss detection and corrective retry, so prose-only reviewer output is never silently treated as clean.
- scope: out of scope: Do not implement baseline-red validation enforcement.
- scope: out of scope: Do not implement frozen-edit detection for task validation changes.
- scope: out of scope: Do not change execute run behavior, add a topological execute loop, add tasks-state.json, context-pack resolution, execute.loop.max, single-writer leases, or --task.
- scope: out of scope: Do not change gate, code_review, memory, or existing pactum review plan behavior.
- scope: out of scope: Do not make plan_review a convergence loop and do not invoke any fixer from plan_review.
- review_resolution: f_001 resolved: Plan review can exit clean after a reviewer still fails to emit the mandatory findings block on the corrective retry.; resolution: After corrective retry, if blocks2 is empty, runPlanReviewerWithRetry now returns an error instead of nil findings. Changed in internal/app/plan_review.go (~line 284).
- review_resolution: f_002 resolved: Plan review silently treats a reviewer response that still lacks the mandatory findings block after corrective retry as a clean review.; resolution: Same fix as f_001: the corrective-retry path now explicitly errors when the response still has no valid pactum.reviewer_findings.v1alpha1 block, preventing silent clean-exit.
- review_resolution: f_003 resolved: The plan-review prompt contradicts the contract by instructing no-issue reviewers not to include the mandatory findings block.; resolution: writePlanReviewerOutputFormat now says 'Always include exactly one findings block. Use an empty array when there are no issues.' instead of the old contradictory 'Do not include an empty findings block.' (internal/app/plan_review.go:485).
- review_resolution: f_004 resolved: Plan-review findings do not enforce the promised lens enum before writing JSON output.; resolution: Added isValidPlanReviewLens helper and a skip in planFindingsFromBlocks for any finding whose lens is not in planReviewLenses. Invalid-lens findings are silently dropped before JSON output (internal/app/plan_review.go).
- review_resolution: f_005 resolved: The no-findings plan-review test treats a missing structured findings block as a clean review, instead of requiring an empty pactum.reviewer_findings.v1alpha1 block.; resolution: TestPlanReviewHelperProcess now emits an empty pactum.reviewer_findings.v1alpha1 block (via planReviewerEmptyFindingsOutput) when PACTUM_PLAN_REVIEWER_EMIT_FINDINGS is unset; updated test comment accordingly. TestPlanReviewNoFindingsExitsZero still expects exit 0 — the reviewer now satisfies the mandatory block requirement with an empty findings array (internal/app/plan_review_test.go).
- review_resolution: f_006 resolved: There is no regression test that pipeline.plan_review.by accepts and runs multiple configured reviewer agents.; resolution: Added TestPlanReviewMultipleReviewers (plan_review_test.go) which configures helper1 and helper2, verifies per-reviewer prompt.txt and attempt-1.txt artifacts, and confirms findings.json contains entries from both agents. Also added plan_review to the accepted panel stages in TestReadConfigPipelineMultiAgentByValidation (config_test.go).
- review_resolution: f_007 resolved: Plan review silently treats missing structured reviewer output as a clean review.; resolution: Removed the early-return guard 'if len(stdoutBytes1) == 0 { return []planReviewFinding{}, nil }' in runPlanReviewerWithRetry; empty stdout now falls through to the corrective retry path just like prose-only output (internal/app/plan_review.go:260).
- review_resolution: f_008 resolved: The new plan-review CLI and pipeline stage are user-visible but are not documented in the user-facing workflow/config docs.; resolution: Updated docs/flow.md: added pactum plan review to the stages table Contract row, and added a full prose paragraph documenting the plan review command, pipeline.plan_review.by config, the five lenses, the mandatory findings block requirement, artifact layout, exit codes, and --json output.
- review_resolution: f_009 resolved: The new VACUOUS_VALIDATION scoped-validation rule is not documented in the user-facing plan documentation.; resolution: Updated docs/flow.md Contract section to document the VACUOUS_VALIDATION rule: tasks with non-empty expected_files must have at least one validation command scoped to those files; global-only commands are rejected with VACUOUS_VALIDATION; tasks with empty expected_files are exempt.
...
- validation: go test ./internal/app -run 'Contract|Plan|Config|Review' passed
