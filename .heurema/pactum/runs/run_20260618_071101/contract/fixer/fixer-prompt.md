...
Current contract version: 4a42c540b4642838f3ec759ba1340bf29b477d8214b599120dbad7a5b84f7517
...
1. [opus/completeness] The `pactum plan review --json` finding-object schema (and the findings.json element schema) has no defined derivation from the reused pactum.reviewer_findings.v1alpha1 capture format. AC 12 requires each finding to expose `lens` (one of five plan lenses), `title`, `description`, and `severity` in {blocking, suggestion}; but the existing captured finding (findingCore at internal/app/review.go:172 + Evidence) carries message/severity(low|medium|high|critical)/category/blocking/evidence and NO `lens` field — lens lives on the attempt (reviewLoopAttemptRef.Lens, review_loop.go:121) via per-lens fan-out. AC 10 mandates 'exactly one' block per reviewer documenting all five lenses, which precludes that per-lens fan-out, leaving lens attribution, the severity value-space collapse, the title/description source, and the findings.json element shape (AC 9c: 'aggregated findings as a JSON array') all unspecified. Since AC 9/12 are the gatable output interface whose artifacts 'must be verified in tests', the missing mapping makes those criteria not deterministically verifiable.
   Evidence: AC 12: finding object fields 'agent ... lens (one of: granularity, dependency-correctness, testability, non-vacuity, scope-fidelity), title, description, and severity (blocking or suggestion)'; In scope item 2 / AC: 'Reuse the EXISTING reviewer-findings capture machinery (the mandatory pactum.reviewer_findings.v1alpha1 block ...)'; AC 10 requires 'exactly one pactum.reviewer_findings.v1alpha1 JSON block'. Repo: findingCore fields message/severity/category/file/line/blocking/confidence (internal/app/review.go:172) + evidence (review_proposals.go:33); lens is on reviewLoopAttemptRef, not the finding (review_loop.go:118-122).
2. [opus/testability] Acceptance criterion 12 (the `--json` per-finding object) is not deterministically testable as written. It requires each finding to expose `agent`, `lens` (one of five enumerated values), `title`, `description`, and `severity` (`blocking`|`suggestion`), but the contract mandates reusing the `pactum.reviewer_findings.v1alpha1` capture schema whose per-finding fields are message/severity(low-critical)/category/blocking/evidence — with no `lens`, `title`, or `description` and a different `severity` enum. Unlike contract_review (one attempt per lens, so lens comes from attempt context), this slice is a single-pass panel where one block covers all five lenses, so there is no per-finding source for `lens` (nor for `title`/`description`, nor a stated `severity` remap). A helper-process reviewer test — the testing approach the contract itself prescribes — emits only the v1alpha1 fields, so it cannot assert expected values for these output fields, leaving the criterion shape-checkable but not outcome-checkable, and the JSON contract ungatable for downstream consumers. Pin the derivation of each `--json` finding field (especially how `lens` is assigned for an all-lenses single attempt) so the criterion becomes a deterministic assertion.
   Evidence: Criterion 12: 'Each finding object has string fields: agent ..., lens (one of: granularity, dependency-correctness, testability, non-vacuity, scope-fidelity), title, description, and severity (blocking or suggestion).' vs. scope item 2 'Reuse the EXISTING reviewer-findings capture machinery (the mandatory pactum.reviewer_findings.v1alpha1 block ...)' whose schema (internal/app/review_proposals.go:69 reviewerFindingProposalInput: message/severity/category/file/line/blocking/confidence/evidence) has no lens/title/description; combined with criterion 10 'requires exactly one pactum.reviewer_findings.v1alpha1 JSON block' (single all-lenses attempt, so lens is not derivable from per-attempt context as it is in contract_review).
3. [codex-xhigh/completeness] The contract includes persisting plan-review "results" in scope, but the acceptance criteria do not define what a result artifact is, where it is written, or how tests should verify it. Clarify whether "results" means the aggregated findings.json, a per-agent parsed result artifact, a command summary, or remove it from scope.
   Evidence: Scope in: "Persist plan-review prompts, attempts, results, and structured findings under a run-local plan-review artifact area." Acceptance only requires prompt artifacts, raw-response artifacts, and plan-review/findings.json.
4. [codex-xhigh/assumptions-surfaced] The contract assumes the existing reviewer-findings capture and corrective-retry machinery can be reused unchanged for plan_review, but it does not explicitly state whether that machinery is generic enough to accept plan-review-specific lenses, finding fields, artifact paths, and exit-code behavior. If the existing capture path is coupled to contract_review semantics, implementers may either fork it incorrectly or silently lose plan-review findings.
   Evidence: Scope in: "Use the existing reviewer_findings structured-output capture behavior, including parse-miss detection and corrective retry"; Acceptance: "Each finding object has string fields: agent, lens ... severity"
...
```json
{
  ...
  "base_version": "4a42c540b4642838f3ec759ba1340bf29b477d8214b599120dbad7a5b84f7517",
  "contract": {
