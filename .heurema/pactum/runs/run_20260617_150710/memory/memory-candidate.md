...
## Review Decisions
- f_001 [medium] resolved internal/app/usage_test.go:297: New cache/effective usage fields are not tested for --all or the non-stage --by modes.
  Resolution: Extended TestUsageSummaryAll (line 302-309) to add CacheReadTokens/CacheCreationTokens to both cross-run records, then assert Totals.CacheReadTokens, Totals.CacheCreationTokens, Totals.CacheReadRatio, and Totals.EffectiveUnits for the --all path. Extended TestUsageSummaryByDimensions (line 357-365, 372-377) to add cache tokens to both records and to the test-case table (wantCacheRead [2]int64 per --by mode), then assert per-group CacheReadTokens and EffectiveUnits inside the existing loop — covering stage, model, agent, and provider dimensions. All 24 Usage tests pass.
- f_002 [low] resolved docs/token-efficiency-research.md:51: `docs/token-efficiency-research.md` still says `pactum usage` shows raw input instead of cache-adjusted cost and that `cache_read` is not surfaced, but the implementation now exposes cache fields, cache ratio, and effective units in both JSON and human output.
  Resolution: docs/token-efficiency-research.md updated: recommendation #1 (surface cache-adjusted cost) marked DONE in this slice.
- Proposal summary: pending=0 accepted=2 rejected=0

## Reusable Project Knowledge
- scope: in scope: Update the existing pactum usage summary path in internal/app/usage.go to aggregate cache_read_tokens, cache_creation_tokens, cache_read_ratio, and effective_units for totals and for every --by group.
- scope: in scope: Update pactum usage human output to show cache read/create information, cache read percentage, effective units, and captured counts for each group and the TOTAL row while preserving coverage and lower-bound reporting.
- scope: in scope: Update pactum usage --json output additively so totals and each group include cache_read_tokens, cache_creation_tokens, cache_read_ratio, and effective_units without removing or renaming existing pactum.usage_summary.v1alpha1 fields.
- scope: in scope: Add or update internal/app usage tests covering run-level, --all, --by stage|model|agent|provider, uncaptured records, zero-input cache ratio, JSON fields, and human table output.
- scope: out of scope: Changing the usage-recording path, ledger schema, TokenUsage capture, or appendUsageRecord behavior.
- scope: out of scope: Changing effective_units multipliers or formula.
- scope: out of scope: Adding dollar costs, price lookup, budget logic, or provider pricing features.
- scope: out of scope: Changing commands other than pactum usage, including status output.
- scope: out of scope: Changing existing --all, run_id resolution, --by, coverage, warning, group sorting, or lower-bound semantics except as needed to display the new fields.
- review_resolution: f_001 resolved: New cache/effective usage fields are not tested for --all or the non-stage --by modes.; resolution: Extended TestUsageSummaryAll (line 302-309) to add CacheReadTokens/CacheCreationTokens to both cross-run records, then assert Totals.CacheReadTokens, Totals.CacheCreationTokens, Totals.CacheReadRatio, and Totals.EffectiveUnits for the --all path. Extended TestUsageSummaryByDimensions (line 357-365, 372-377) to add cache tokens to both records and to the test-case table (wantCacheRead [2]int64 per --by mode), then assert per-group CacheReadTokens and EffectiveUnits inside the existing loop — covering stage, model, agent, and provider dimensions. All 24 Usage tests pass.
- review_resolution: f_002 resolved: `docs/token-efficiency-research.md` still says `pactum usage` shows raw input instead of cache-adjusted cost and that `cache_read` is not surfaced, but the implementation now exposes cache fields, cache ratio, and effective units in both JSON and human output.; resolution: docs/token-efficiency-research.md updated: recommendation #1 (surface cache-adjusted cost) marked DONE in this slice.
