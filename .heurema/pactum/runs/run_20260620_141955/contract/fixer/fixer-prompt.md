...
Current contract version: 080ea53e9dbf3aa81175036c137897e0d865ed0494de161d2eb61bc71477ccfa
...
1. [opus/completeness] AC5 is not specific enough to gate the current_code_only=false + blocking=true behavior: 'cannot produce a persisted blocking proposal or finding ... emits a warning' is satisfied by both dropping the finding entirely and keeping it as a downgraded non-blocking finding. These are materially different outcomes for a rule the goal states prominently ('a current_code_only=false finding must not be blocking'), and the gate cannot distinguish the intended implementation from the unintended one.
   Evidence: Acceptance criterion: 'A reviewer output item with current_code_only=false and blocking=true cannot produce a persisted blocking proposal or finding, and collection emits a warning for that invalid combination.' — drop vs downgrade is unresolved.
2. [opus/assumptions-surfaced] Implicit assumption: the contract never states whether a finding with state="candidate" may be persisted as blocking. Since approval is gated on blocking findings, this silently decides whether uncertain candidates can block a merge — the exact false-positive the recall-first reframe targets. Both behaviours pass every acceptance criterion, so the intent is unverifiable. Make the state-vs-blocking relationship explicit.
   Evidence: Goal/Scope: blocking is constrained only by 'a blocking finding must have current_code_only=true'; no rule ties blocking to state. Prompt reframe asks reviewers to 'report tightly-evidenced candidates, state uncertainty'. Acceptance criteria test only the current_code_only constraint, not state-vs-blocking.
3. [codex-xhigh/assumptions-surfaced] The contract does not explicitly state whether current_code_only is required for code-review findings or how to distinguish an omitted boolean from an explicit false. This matters because a Go bool zero value can make missing current_code_only look like current_code_only=false, changing validation behavior.
   Evidence: New fields include "current_code_only (bool)", but validation only says: "require non-empty trigger, evidence, fix_direction and a valid state".
4. [codex-xhigh/assumptions-surfaced] The contract leaves implicit whether a finding with current_code_only=false and blocking=true should be dropped entirely or persisted as non-blocking with a warning. Both satisfy 'cannot produce a persisted blocking proposal', but they produce different records and tests.
   Evidence: "A reviewer output item with current_code_only=false and blocking=true cannot produce a persisted blocking proposal or finding, and collection emits a warning for that invalid combination."
...
```json
{
  ...
  "base_version": "080ea53e9dbf3aa81175036c137897e0d865ed0494de161d2eb61bc71477ccfa",
  "contract": {
