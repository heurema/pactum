...
## Review Decisions
- f_001 [low] open docs/agent-skill.md:48: The user-facing agent skill overview still hardcodes Codex for the safe execute-plan step, so the documentation path can still steer non-Codex users to the old command after the lifecycle affordances were de-hardcoded.
- Proposal summary: pending=0 accepted=1 rejected=0

## Reusable Project Knowledge
- scope: in scope: Change internal/app/resolve.go so the prompt_built lifecycle next affordance emits pactum execute plan <runID> without --agent codex.
- scope: in scope: Change internal/app/errors.go so noExecutionAttemptError emits pactum execute plan <runID> without --agent codex.
- scope: in scope: Change internal/app/prompt.go so the human Next: line after prompt build prints pactum execute plan <runID> without --agent codex.
- scope: in scope: Update internal/app/prompt_test.go and internal/app/affordances_test.go assertions for these lifecycle affordance and human-output surfaces to expect the bare execute-plan command.
- scope: out of scope: Changing execute-plan default agent resolution, including prepareExecution or ExecutePlan behavior.
- scope: out of scope: Changing CLI schema, flags, prompt manifest structure, or adding a placeholder agent token to machine next arrays.
- scope: out of scope: Changing internal/app/skill.go install-target help text or unrelated skill-install tests that legitimately mention --agent codex.
- scope: out of scope: Changing docs, SKILL.md guidance, or contract-review calibration behavior.
...
- validation: sh -c 'if rg -n "pactum execute plan .*--agent codex" internal/app/resolve.go internal/app/errors.go internal/app/prompt.go internal/app/prompt_test.go internal/app/affordances_test.go; then exit 1; fi' passed
