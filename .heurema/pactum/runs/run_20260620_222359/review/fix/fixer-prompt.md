...
## Inputs
- Fixer context: .heurema/pactum/runs/run_20260620_222359/review/fix/fixer-context.md
- Contract: .heurema/pactum/runs/run_20260620_222359/contract/contract.json
- Review artifacts: .heurema/pactum/runs/run_20260620_222359/review/review.json, .heurema/pactum/runs/run_20260620_222359/review/findings.jsonl, .heurema/pactum/runs/run_20260620_222359/review/resolutions.jsonl
...
- Summary: findings=10 open=10 resolved=0 blocking_open=9
- Blocking findings (fix or rebut these — emit exactly one fix-outcome for each):
  - f_001 severity=high category=correctness blocking=true status=open: Gate skips guarded attempts, so a later blocked git-guard attempt can be hidden by an older successful attempt for the same contract.
    location: internal/app/gate.go:288
  - f_002 severity=high category=correctness blocking=true status=open: The guard does not snapshot commit objects or non-HEAD reflogs, allowing some git-history mutations to pass as clean.
    location: internal/app/git_guard.go:202
  - f_003 severity=high category=correctness blocking=true status=open: Gate can ignore the latest git-guard-blocked execution by falling back to an older successful attempt.
    location: internal/app/gate.go:288
  - f_004 severity=high category=correctness blocking=true status=open: The guard never snapshots commit object IDs, so commit-object-only history mutations can pass as clean.
    location: internal/app/git_guard.go:177
  - f_005 severity=medium category=correctness blocking=true status=open: Reflog detection is incomplete and can miss reflog-only mutations.
    location: internal/app/git_guard.go:202
  - f_006 severity=medium category=correctness blocking=true status=open: Prechecks miss loose ref lock files, so unsupported locked ref state can still run the agent.
    location: internal/app/git_guard.go:105
  - f_007 severity=medium category=quality blocking=true status=open: Git-guard lifecycle behavior is not covered by integration tests.
    location: internal/app/agent_attempt.go:184
  - f_008 severity=medium category=quality blocking=true status=open: Several contract-required git-guard edge cases are missing focused tests.
    location: internal/app/git_guard_test.go:48
  - f_009 severity=high category=correctness blocking=true status=open: The gate silently skips a git-guard-blocked attempt and can fall back to an older completed attempt for the same contract.
    location: internal/app/gate.go:288
- Advisory (non-blocking) findings (context only — do NOT edit code and do NOT emit outcomes for them):
  - f_010 severity=medium category=quality blocking=false status=open: The user-facing write-stage docs were not updated for the new git-history guard behavior.
    location: docs/agents.md:264
