...
Current contract version: da01a459b69d09c02cc6294562b1838a8a89acb87e36a553abcedac56e7e9a94
...
1. [opus/completeness] When an inconclusive condition co-occurs with a restorable ref/index mutation in the same run, the combined-precedence criterion ('all detected mutations that are classifiable as restorable are still restored before the dominant reason is recorded') contradicts the inconclusive/stash safety rule ('do NOT auto-restore in v1' / 'take no action that would make the dangling work harder to recover'). The contract does not specify whether restoration is suppressed when inconclusive is the dominant reason.
   Evidence: Combined criterion: 'all detected mutations that are classifiable as restorable are still restored before the dominant reason is recorded'; vs DESIGN 4 inconclusive: 'do NOT auto-restore in v1 — the work is preserved in the reflog/dangling commits'; vs stash criterion: 'the guard takes no action ... that would make the stashed or dangling work harder to recover.'
2. [opus/assumptions-surfaced] The execute clean-tree precondition assumes the guarded working tree is free of pactum's own run artifacts, but pactum writes a tracked ledger file and an untracked run directory into that tree, which would make porcelain non-empty and bail every execute attempt as inconclusive before the agent runs.
   Evidence: Design step 2 / acceptance: execute precondition requires `git status --porcelain=v1 -z --untracked-files=all` empty else `executor_git_guard_inconclusive` without running the agent. Session git status shows `M .heurema/pactum/ledger/events.jsonl` and `?? .heurema/pactum/runs/run_20260620_222359/` — both surface in that exact porcelain invocation (`--untracked-files=all`).
3. [codex-xhigh/completeness] The contract requires restoring changed refs' reflogs, but the snapshot only requires storing reflog line counts or hashes, which is insufficient data to restore the original reflog contents.
   Evidence: SNAPSHOT: "reflog state of HEAD and all refs (line count or hash...)"; RESTORE: "restore any other changed refs and their reflogs from the snapshot LAST".
4. [codex-xhigh/scope-fidelity] The scope is under-broad for the reset-hard/no-work-loss guarantee because it does not include any working-tree content backup or recovery mechanism.
   Evidence: Scope in snapshots git metadata such as refs, reflogs, commit object ids, and the index tree hash; scope out says not to auto-restore ambiguous reset-hard cases; acceptance requires "a `reset --hard`/stash case is detected and finishes `executor_git_guard_inconclusive` WITHOUT losing the work" and says the guard should "NEVER silently lose the agent's work".
...
```json
{
  ...
  "base_version": "da01a459b69d09c02cc6294562b1838a8a89acb87e36a553abcedac56e7e9a94",
  "contract": {
