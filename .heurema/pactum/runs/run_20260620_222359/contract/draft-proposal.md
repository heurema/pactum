...
- Drafter: codex
- Accepted by: manual
- Accepted at: 2026-06-20T22:26:43Z
...
- If the agent creates commits, amends commits, rebases, or moves the current branch from a clean start, the guard detects the history mutation, restores HEAD and the current branch to the original commit with `git reset --mixed`, leaves the agent's file changes as unstaged working-tree changes, and records `terminal_reason` `executor_git_history_mutation`.
...
- If HEAD hash ends unchanged but reflog state or reachable/dangling commit objects changed, including commit-then-reset-hard, the guard detects the mutation and records `terminal_reason` `executor_git_guard_inconclusive` without silently proceeding.
- If a new stash entry is created, the guard detects it, does not auto-restore in v1, leaves recoverable git state intact, and records `terminal_reason` `executor_git_guard_inconclusive`.
- If any guard restore command fails, the attempt records `terminal_reason` `executor_git_restore_failed` and includes enough restore error detail for a human to diagnose the repository state.
...
- Tests assert precondition no-run behavior, clean no-op behavior, commit restore, index-only restore, ref mutation handling, reset-hard or stash inconclusive handling, result artifact fields, next suppression, gate blocking, and read-only stage non-interference.

## Validation commands
- go test ./internal/app -run 'GitGuard|Execute|ReviewFix|Gate' -count=1
- go test ./internal/app ./internal/gitctx -count=1
- make check
...
- Rejecting write-enabled agent execution when the repository has any pre-existing staged, unstaged, or untracked change is acceptable for v1.
