...
- Questions:
  - q_001 blocking=true status=open: What exactly counts as a 'mutating command' for the required `next` array: every command that writes anything (`init`, `map refresh`, `task use`, `export`, `memory refresh`, etc.), or only workflow/run-stage commands such as `task new`, `contract approve`, `prompt build`, `execute plan/run`, `gate run`, `review ...`, and `memory propose/accept`?
    ...
    recommended answer (confidence medium): Define `mutating command` as commands that create or update Pactum workflow state and already support `--json`; exclude pure read-only commands and exclude `export` because it writes an archive but does not advance the Pactum state machine. For in-scope mutating commands with no meaningful next action, emit `next: []`.
  - q_002 blocking=true status=open: For the JSON error envelope, should the stable machine-readable field be named `error.code` as in the existing `pactum.error.v1` implementation, or `error.reason` as described in the backlog phrase '`reason` + `fix`'?
    kind: terminology
    ...
    recommended answer (confidence medium): Keep `error.code` as the canonical reason-code field for compatibility with the existing `pactum.error.v1` envelope, and add `error.fix` when an exact remedial command exists. Do not add a parallel `error.reason` field in this slice.
  - q_003 blocking=true status=open: Should existing JSON affordance fields such as `suggested_command` and `next_command` remain for compatibility while the new `fix` and `next` fields are added, or should they be removed/renamed immediately?
    kind: assumption
    ...
    recommended answer (confidence medium): Preserve existing `suggested_command` and `next_command` fields where they already exist, add the new `fix` and `next` fields, and update the bundled skill/docs to prefer `fix` and `next`.
    depends on: q_002
    blocked: waiting on unanswered prerequisites
  - q_004 blocking=true status=open: How should read-only not-ready responses behave, such as `gate show --json` before a gate report exists or `review show --json` before review is prepared: keep the current exit-0 `pactum.not_ready.v1` response, or convert them to nonzero `pactum.error.v1` envelopes?
    kind: scope
    ...
    recommended answer (confidence high): Keep read-only not-ready responses exit 0 and keep `pactum.not_ready.v1`; add `fix` mirroring the exact remedial command while preserving `suggested_command` for compatibility.
    depends on: q_002, q_003
    blocked: waiting on unanswered prerequisites
  - q_005 blocking=true status=answered: For the 'no completed execution attempt' precondition on `pactum gate run --json`, should `fix` ever suggest `pactum execute run`, even though real agent execution is unsandboxed and requires explicit human approval?
    kind: edge_case
    ...
    recommended answer (confidence high): Do not put `pactum execute run` in `fix`. For this precondition, omit `fix` and use the human message to say a completed execution attempt is required; `next` may point to the safe preparation command `pactum execute plan <run_id> --agent codex`.
  ...
  - q_006 blocking=true status=open: When there is no single exact remedial command, such as multiple open blocking clarification questions, multiple pending review proposals, or no current run with multiple active runs, should `fix` be omitted, set to an empty string, or contain a command with placeholders?
    kind: edge_case
    ...
    recommended answer (confidence high): Make `fix` optional and omit it when no single exact runnable command exists. Never emit placeholder commands in `fix`; keep multi-step guidance in the human-readable message and expose concrete legal commands through `next` only when they are directly runnable.
    depends on: q_002
    blocked: waiting on unanswered prerequisites
  - q_007 blocking=false status=open: Which precondition failures must be pinned by acceptance tests for this slice?
    kind: acceptance
    ...
    recommended answer (confidence medium): Pin at least: `not_initialized`, `project_map_stale`, `contract_not_approved`, `blocking_clarifications_open`, `prompt_not_built`, `no_execution_attempt`, `gate_report_missing`, `review_not_prepared`, `pending_review_proposals`, and `run_not_found`; each test should assert schema, code, message, optional fix, unchanged exit code, and empty stderr in `--json` mode.
    depends on: q_001, q_002, q_004, q_005, q_006
    blocked: waiting on unanswered prerequisites
  - q_008 blocking=true status=open: Besides mutating commands, should the new plural `next` array be added to read-only JSON responses that already expose next-step affordances, specifically `pactum task show --json`, `pactum task list --json`, and `pactum status --json`?
    kind: scope
    ...
    recommended answer (confidence medium): Add plural `next` to `pactum status --json` and `pactum task show --json` because they are current-run guidance surfaces; leave `pactum task list --json` unchanged except for preserving existing per-run `next_command` fields because it is an inventory view and does not print a human `Next:` block.
  - q_009 blocking=true status=open: Where should the new plural `next` array live in JSON responses: as a top-level `next` field on each command response, or nested beside existing affordance fields such as `runs.next_command` and per-run `next_command`?
    kind: acceptance
    ...
    recommended answer (confidence medium): Use a top-level `next: []` array on each in-scope command response, including `pactum status --json`; preserve existing `next_command` fields for compatibility where they already exist.
    depends on: q_008
    blocked: waiting on unanswered prerequisites
  - q_010 blocking=true status=open: For JSON `next`, should Pactum mirror the human `Next:` block byte-for-byte, or normalize it into concrete directly runnable commands when the human hint is bare, multi-step, or contains placeholders such as `<question_id>` and `"<answer>"`?
    kind: terminology
    ...
    recommended answer (confidence medium): Define JSON `next` as concrete directly runnable command strings. It may fill known run ids and omit placeholder-only templates; human output can keep its existing explanatory `Next:` text. Preserve exact set/order only where the human hints are already concrete commands.
    depends on: q_009
    blocked: waiting on unanswered prerequisites
  - q_011 blocking=false status=open: When the current stage has no safe automatic next command because human input is required, for example open blocking clarification questions after `task new --clarify`, should `next` be empty or contain a safe inspection command such as `pactum clarify status <run_id>`?
    kind: edge_case
    ...
    recommended answer (confidence medium): Emit only safe, concrete commands in `next`; for open blocking clarifications, prefer `next: ["pactum clarify status <run_id>"]` and keep answer templates in the human-readable message, not in `next`.
    depends on: q_010
    blocked: waiting on unanswered prerequisites
