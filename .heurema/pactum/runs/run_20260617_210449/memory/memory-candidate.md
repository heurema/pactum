...
## Review Decisions
- f_001 [high] resolved internal/app/review_loop.go:637: A reviewer lens transport failure can still be counted as a clean round when the remaining lenses emit valid empty findings blocks.
- f_002 [high] resolved internal/app/review.go:1271: The corrective retry prompt omits the reviewer context and artifact paths, so a fresh retry has no basis to re-review the task.
- f_003 [medium] resolved internal/app/review_proposals.go:443: The multiple-block parse-miss check ignores malformed or missing-findings schema blocks when one sibling block is valid.
- f_004 [high] resolved internal/app/review_proposals_test.go:57: The parser test still encodes the old silent-drop behavior for prose-only reviewer output.
- f_005 [high] resolved internal/app/review_loop_test.go:1902: The review-loop tests do not exercise the new corrective retry path for successful reviewer attempts with structurally invalid output.
- f_006 [medium] resolved internal/app/review_test.go:2207: The prompt contract test does not assert the mandatory-output requirements added by this change.
- f_007 [medium] resolved internal/app/review_test.go:1855: Proposal field-validation tests do not explicitly cover the required-field rejection contract.
- f_008 [high] resolved internal/app/review_loop.go:267: Mixed reviewer rounds can still proceed after a per-lens parse miss when another lens produced proposals.
- f_009 [medium] resolved internal/app/review.go:1161: The generated reviewer prompt still describes structured proposal blocks as optional, contradicting the new mandatory findings-block contract.
- f_010 [medium] resolved docs/agents.md:545: The operator documentation still says reviewer structured findings are optional, but review runs now require a valid findings block and retry or escalate when it is missing.
- Proposal summary: pending=0 accepted=10 rejected=0

## Reusable Project Knowledge
- scope: in scope: Harden `renderReviewerPrompt` so every reviewer lens prompt makes the fenced `pactum.reviewer_findings.v1alpha1` JSON block mandatory, always emitted, and the only parsed reporting channel.
- scope: in scope: Include a worked clean reviewer-output example with `"findings": []` and state that prose is supplemental only and ignored by the parser.
- scope: in scope: Change reviewer finding block parsing so an explicit empty `findings` array is distinguishable from a missing `findings` key.
- scope: in scope: Treat no valid reviewer findings block, malformed block JSON, or a schema block missing `findings` as a structural parse miss with a warning.
- scope: in scope: Enforce the valid-block requirement per reviewer lens attempt, including mixed rounds where some lenses emit proposals and another lens emits no valid block.
- scope: in scope: Add a bounded per-lens corrective retry for missing or invalid findings blocks before escalating to `reviewer_findings_unparsed`.
- scope: in scope: Keep corrective retry accounting below the outer review-loop round accounting so `MaxRounds`, patience, and clean-round logic still count logical review rounds.
- scope: in scope: Update focused Go tests and review-loop helper fixtures so clean reviewer output emits an explicit `"findings": []` block.
- scope: in scope: Harden the parser against structurally ambiguous reviewer output: more than one fenced pactum.reviewer_findings.v1alpha1 block in an attempt is a parse miss; per-finding field validation excludes entries missing required fields (message/severity/category) with a warning while still capturing valid sibling findings in the same block.
- scope: out of scope: Changing the contract goal.
- scope: out of scope: Routing or defaulting review to a different reviewer agent or model.
- scope: out of scope: Parsing prose findings into proposals.
- scope: out of scope: Changing fixer behavior, gate behavior, or proposal auto-accept semantics beyond preventing silently partial review rounds.
- scope: out of scope: Running real reviewer or fixer agents as part of validation.
- review_resolution: f_001 resolved: A reviewer lens transport failure can still be counted as a clean round when the remaining lenses emit valid empty findings blocks.
- review_resolution: f_002 resolved: The corrective retry prompt omits the reviewer context and artifact paths, so a fresh retry has no basis to re-review the task.
- review_resolution: f_003 resolved: The multiple-block parse-miss check ignores malformed or missing-findings schema blocks when one sibling block is valid.
- review_resolution: f_004 resolved: The parser test still encodes the old silent-drop behavior for prose-only reviewer output.
- review_resolution: f_005 resolved: The review-loop tests do not exercise the new corrective retry path for successful reviewer attempts with structurally invalid output.
- review_resolution: f_006 resolved: The prompt contract test does not assert the mandatory-output requirements added by this change.
- review_resolution: f_007 resolved: Proposal field-validation tests do not explicitly cover the required-field rejection contract.
- review_resolution: f_008 resolved: Mixed reviewer rounds can still proceed after a per-lens parse miss when another lens produced proposals.
- review_resolution: f_009 resolved: The generated reviewer prompt still describes structured proposal blocks as optional, contradicting the new mandatory findings-block contract.
- review_resolution: f_010 resolved: The operator documentation still says reviewer structured findings are optional, but review runs now require a valid findings block and retry or escalate when it is missing.
...
- validation: go test ./internal/app -run Review passed
