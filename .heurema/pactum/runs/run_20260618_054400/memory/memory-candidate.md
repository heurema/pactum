...
## Review Decisions
- f_001 [medium] resolved internal/app/contract_draft.go:270: ContractAcceptDraft returns planValidationError directly, so accepting an invalid drafted plan loses the structured plan validation issue codes.
  Resolution: Added `errors` import to contract_draft.go and added a planValidationError type-check in ContractAcceptDraft (after line ~270). When contractReviseWithUpdate returns a planValidationError, the structured failure (with Issues carrying the error codes) is written to stdout via writeReviseFailure, and contractReviseFailureError{} is returned — matching the pattern used in ContractRevise.
- f_002 [medium] resolved internal/app/contract_draft.go:270: Accepting an invalid drafted plan rejects the change but drops the existing plan validation issue details, so callers do not see codes like CYCLE_IN_DAG or MISSING_DEPENDENCY as required.
  Resolution: Same fix as f_001: the planValidationError handler in ContractAcceptDraft now emits the full contractReviseFailure JSON (including issue Codes like CYCLE_IN_DAG / MISSING_DEPENDENCY) to stdout rather than returning the raw error, ensuring callers can read the structured issue codes.
- f_003 [medium] resolved internal/app/plan_test.go:309: The drafted-plan round-trip test bypasses the drafter output parsing and proposal-recording path by writing contractDraftProposalDocument directly, so the new plan field on contractDraftProposalBlock and recordContractDraftProposal are not tested for plan.tasks[] input.
  Resolution: Rewrote the proposal setup in TestDraftedPlanProposalRoundTrip to construct a fenced JSON block (as a drafter would emit), then parse it through parseContractDraftProposalBlocks and contractDraftProposalFromBlock, then persist via writeContractDraftProposalArtifacts. The test now asserts that parseContractDraftProposalBlocks picks up the plan field from the block and that contractDraftProposalFromBlock carries it into the proposal document. Added time import for time.Now().UTC().
- f_004 [medium] resolved internal/app/plan_test.go:505: The invalid drafted-plan accept tests only require any non-zero exit and do not assert the required actionable plan validation issue code.
  Resolution: Added JSON parsing of stdout after the non-zero exit check in both subtests of TestAcceptInvalidDraftedPlanFails. The cycle subtest asserts CYCLE_IN_DAG is present in failure.Issues; the unresolved_dep subtest asserts MISSING_DEPENDENCY. These assertions work because the f_001/f_002 fix now writes the structured JSON failure to stdout before returning.
- f_005 [medium] resolved internal/app/plan_test.go:431: The plan show --json tests do not verify that all task fields are emitted.
  Resolution: Extended the plan show --json section in TestAcceptDraftedPlanAppliesItToContract to assert all task fields: t1 title, context path and why, expected_files, acceptance, validation; t2 depends_on, acceptance, validation — matching the task data set up in the test's proposal.
- f_006 [medium] resolved internal/app/plan_test.go:181: The read-only test for plan show does not cover all records named by the contract.
  Resolution: Extended readState in TestPlanShowIsReadOnly to capture two additional records: gate/gate-report.json and review/review.json (via mustReadFileOrAbsent, returning '<absent>' if not present). Added gateBefore/gateAfter and reviewBefore/reviewAfter comparisons after the plan show invocations.
- f_007 [low] resolved docs/flow.md:18: The new user-visible `pactum plan show` command is not documented in the workflow command table, leaving the public command reference stale.
  Resolution: Addressed: added 'pactum plan show' to the docs/flow.md Stages-at-a-glance command table (Contract row) and a sentence in the Contract stage details. Advisory finding — the fixer declines non-blocking findings by design, so this is operator docs completion.
- Proposal summary: pending=0 accepted=7 rejected=0

## Reusable Project Knowledge
- scope: in scope: Add optional plan support to contract draft proposal parsing, persistence, JSON output, markdown preview, and summaries so draft proposals can round-trip plan.tasks[] before acceptance.
- scope: in scope: Extend the contract drafter prompt/context to document optional plan emission rules and every supported task field without enforcing task granularity in code.
- scope: in scope: Update ContractAcceptDraft so accepted draft plans populate contractPartialUpdate.Plan, count as a real change even when plan is the only proposed field, and pass through existing plan validation.
- scope: in scope: Add a read-only pactum plan show [run_id] [--json] command that renders the contract's static plan DAG in text and JSON and is registered in CLI help/command parsing.
- scope: in scope: Add focused internal/app tests for drafted plan parsing, accepted plan persistence, invalid drafted plan rejection, plan show text/json/no-plan output, and plan-less proposal compatibility.
- scope: out of scope: Do not make execute, gate, review, or prompt execution consume plan.tasks[].
- scope: out of scope: Do not add topological execution, tasks-state.json, context-pack resolution, plan_review, validation freeze, single-writer leases, --task, or task-level execution state.
- scope: out of scope: Do not enforce the drafter prompt's task granularity guidance in code; structural plan validation remains the existing slice-1 validation surface.
- scope: out of scope: Do not change the contract goal or existing approved plan validation semantics beyond carrying drafted plans into that validation path.
- review_resolution: f_001 resolved: ContractAcceptDraft returns planValidationError directly, so accepting an invalid drafted plan loses the structured plan validation issue codes.; resolution: Added `errors` import to contract_draft.go and added a planValidationError type-check in ContractAcceptDraft (after line ~270). When contractReviseWithUpdate returns a planValidationError, the structured failure (with Issues carrying the error codes) is written to stdout via writeReviseFailure, and contractReviseFailureError{} is returned — matching the pattern used in ContractRevise.
- review_resolution: f_002 resolved: Accepting an invalid drafted plan rejects the change but drops the existing plan validation issue details, so callers do not see codes like CYCLE_IN_DAG or MISSING_DEPENDENCY as required.; resolution: Same fix as f_001: the planValidationError handler in ContractAcceptDraft now emits the full contractReviseFailure JSON (including issue Codes like CYCLE_IN_DAG / MISSING_DEPENDENCY) to stdout rather than returning the raw error, ensuring callers can read the structured issue codes.
- review_resolution: f_003 resolved: The drafted-plan round-trip test bypasses the drafter output parsing and proposal-recording path by writing contractDraftProposalDocument directly, so the new plan field on contractDraftProposalBlock and recordContractDraftProposal are not tested for plan.tasks[] input.; resolution: Rewrote the proposal setup in TestDraftedPlanProposalRoundTrip to construct a fenced JSON block (as a drafter would emit), then parse it through parseContractDraftProposalBlocks and contractDraftProposalFromBlock, then persist via writeContractDraftProposalArtifacts. The test now asserts that parseContractDraftProposalBlocks picks up the plan field from the block and that contractDraftProposalFromBlock carries it into the proposal document. Added time import for time.Now().UTC().
- review_resolution: f_004 resolved: The invalid drafted-plan accept tests only require any non-zero exit and do not assert the required actionable plan validation issue code.; resolution: Added JSON parsing of stdout after the non-zero exit check in both subtests of TestAcceptInvalidDraftedPlanFails. The cycle subtest asserts CYCLE_IN_DAG is present in failure.Issues; the unresolved_dep subtest asserts MISSING_DEPENDENCY. These assertions work because the f_001/f_002 fix now writes the structured JSON failure to stdout before returning.
- review_resolution: f_005 resolved: The plan show --json tests do not verify that all task fields are emitted.; resolution: Extended the plan show --json section in TestAcceptDraftedPlanAppliesItToContract to assert all task fields: t1 title, context path and why, expected_files, acceptance, validation; t2 depends_on, acceptance, validation — matching the task data set up in the test's proposal.
- review_resolution: f_006 resolved: The read-only test for plan show does not cover all records named by the contract.; resolution: Extended readState in TestPlanShowIsReadOnly to capture two additional records: gate/gate-report.json and review/review.json (via mustReadFileOrAbsent, returning '<absent>' if not present). Added gateBefore/gateAfter and reviewBefore/reviewAfter comparisons after the plan show invocations.
- review_resolution: f_007 resolved: The new user-visible `pactum plan show` command is not documented in the workflow command table, leaving the public command reference stale.; resolution: Addressed: added 'pactum plan show' to the docs/flow.md Stages-at-a-glance command table (Contract row) and a sentence in the Contract stage details. Advisory finding — the fixer declines non-blocking findings by design, so this is operator docs completion.
...
- validation: go test ./internal/app -run 'Contract|Plan|Draft' passed
