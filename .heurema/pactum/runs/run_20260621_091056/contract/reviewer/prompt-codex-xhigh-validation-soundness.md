...
**Acceptance criteria**:
  ...
  - Critic verdict artifacts are written as pactum.review_critic_verdicts.v1alpha1 JSONL with verdicts sorted by proposal_id for deterministic output.
  ...
  - The implementation includes internal/app/review_critic_test.go containing tests covering at minimum: critic verdict parsing (confirmed, disputed, insufficient_evidence, unknown proposal_id); missing-verdict treatment as insufficient_evidence; duplicate-verdict handling; critic prompt content; critic model selection using registered reviewer agents (different-engine default, same-engine fallback with warning, and explicit critic_by); critic agent attempt executed as read-only through runAgentAttemptLifecycle; proposal acceptance filtering for each verdict type; all three new terminal reasons individually and their precedence when co-occurring (critic_verdicts_unparsed > debate_no_consensus > precision_rejected); the approve-gate rejection behavior for debate_no_consensus and critic_verdicts_unparsed; per-round critic fields and artifact recording; pactum review run --json output exposing critic fields; critic execution failure (transport error, empty stdout) terminating as critic_verdicts_unparsed without a corrective re-attempt; intra-batch fingerprint dedup (two proposals in the same batch with identical fingerprints: first is evaluated by the critic, second is treated as duplicate bypassing the critic); the case where one blocking proposal is confirmed and a different blocking proposal has an insufficient_evidence verdict in the same round (debate_no_consensus does not fire; the confirmed finding becomes an open blocker and the loop continues); the observable status of critic-filtered proposals in round summary artifacts (critic_disputed, critic_insufficient_evidence, critic_verdict_missing); and a round where all non-blocking proposals are critic-disputed does not trigger precision_rejected, debate_no_consensus, or an incorrect clean-round terminal reason. The focused validation command uses -v and -count=1 to confirm the tests execute and are not vacuously skipped.
  - When all non-duplicate proposals in a round receive disputed or insufficient_evidence verdicts from the critic (precision_confirmed == 0) and none of the proposals were blocking, no new terminal reason from the critic pass fires. The round is not treated as a clean round; critic-filtered proposals count as evaluated-but-not-accepted proposals and are reflected in the round summary fields (precision_candidates, precision_rejected, precision_unresolved). The loop evaluates the post-critic open-finding state and applies the existing path logic for a round with zero accepted proposals and zero duplicates from this batch (for example: resolved when no open blocking findings remain; no-progress if this state recurs without change).
  ...
  - Critic-filtered proposals (those receiving disputed or insufficient_evidence verdicts, or whose verdict is absent from the critic response per criterion 7) are NOT passed to acceptReviewLoopProposal and do not appear as open, rebutted, or accepted findings in the review state. The round summary JSONL records each critic-filtered proposal with a distinct observable status field: critic_disputed for disputed verdicts, critic_insufficient_evidence for explicit insufficient_evidence verdicts, and critic_verdict_missing for proposals whose verdict was absent from the critic response. These proposals are not persisted as candidate or pending findings and are not visible to the fixer.
