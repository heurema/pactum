...
- Summary: findings=14 open=14 resolved=0 blocking_open=11
- Blocking findings (fix or rebut these — emit exactly one fix-outcome for each):
  - f_001 severity=high category=correctness blocking=true status=open: A mixed critic result with one confirmed blocking proposal and one insufficient-evidence blocking proposal incorrectly terminates as debate_no_consensus before accepting the confirmed blocker.
    location: internal/app/review_loop.go:409
  - f_002 severity=medium category=correctness blocking=true status=open: Same-engine critic fallback selects the first registry entry instead of the resolved first reviewer.
    location: internal/app/review_critic.go:143
  - f_003 severity=medium category=correctness blocking=true status=open: Synthetic missing verdict artifact entries use verdict=insufficient_evidence instead of the required verdict value missing_verdict.
    location: internal/app/review_critic.go:481
  - f_004 severity=high category=correctness blocking=true status=open: A round with both a confirmed blocking proposal and an insufficient-evidence blocking proposal is incorrectly terminated as debate_no_consensus before accepting the confirmed blocker.
    location: internal/app/review_loop.go:409
  - f_005 severity=medium category=correctness blocking=true status=open: Same-engine critic fallback selects the first registry entry instead of reviewers[0].
    location: internal/app/review_critic.go:143
  - f_006 severity=medium category=validation blocking=true status=open: Missing critic verdicts are recorded with the wrong artifact verdict and no warning.
    location: internal/app/review_critic.go:483
  - f_007 severity=medium category=validation blocking=true status=open: Zero-valued precision counters disappear from loop-summary.json and --json output.
    location: internal/app/review_loop.go:108
  - f_008 severity=medium category=scope blocking=true status=open: The critic prompt does not allow explicit insufficient_evidence verdicts.
    location: internal/app/review_critic.go:234
  - f_009 severity=medium category=quality blocking=true status=open: The tests do not cover the required mixed-verdict case where one blocking proposal is confirmed and a different blocking proposal is insufficient_evidence.
    location: internal/app/review_critic_test.go:317
  - f_010 severity=medium category=quality blocking=true status=open: The critic read-only execution requirement is not tested.
    location: internal/app/review_critic_test.go:27
  - f_011 severity=medium category=quality blocking=true status=open: The intra-batch dedup test contains a no-op assertion for the candidate-count behavior it claims to verify.
    location: internal/app/review_critic_test.go:424
- Advisory (non-blocking) findings (context only — do NOT edit code and do NOT emit outcomes for them):
  - f_012 severity=low category=quality blocking=false status=open: The code-review-only critic_by setting is modeled on the shared pipelineStage, so critic_by is accepted under every pipeline stage even though only pipeline.code_review uses it.
    location: internal/app/config.go:87
  - f_013 severity=medium category=quality blocking=false status=open: The active agent configuration docs do not mention the new `pipeline.code_review.critic_by` registry-name override, so users cannot discover how to configure the critic agent.
    location: docs/agents.md:29
  - f_014 severity=medium category=quality blocking=false status=open: The review-run docs are stale for the new critic pass: they still describe direct proposal acceptance and omit the new critic terminal reasons and round-summary fields.
    location: docs/flow.md:248
