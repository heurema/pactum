...
- Approval: approved
- Contract hash: c06595081ae6ec95296de8419493d9e0621e275c527fb1ee9dd608c94811dc58
...
- Contract-review blocking findings are persisted to a findings.jsonl artifact (one JSON object per line, each containing at minimum the fields category, message, and blocking) in the contract-review run directory at the end of the review loop. contract approve reads this artifact from the latest run to enforce the approval guard. If findings.jsonl is absent, unreadable, or malformed, contract approve fails-closed (exits nonzero, refuses approval). This artifact is the sole durable source of truth for the contract-side approval guard; in-memory state is not sufficient.
- The durable source of truth for code-review blocking findings is the latest review run's stored output or summary artifact, which records each finding's blocking field; review approve reads this artifact and refuses while any finding with blocking=true is present. When both stored output and summary artifact are available and disagree on whether any finding has blocking=true, the more conservative reading (refuse approval if either indicates a blocker) governs. When the relevant artifact is absent, unreadable, or malformed, review approve fails-closed (exits nonzero, refuses approval).
- Finding identity for fixer-no-progress streak detection: a finding is treated as a blocking finding for streak-detection purposes when its blocking field is true (independent of its severity value). Its canonical key is the tuple (category, normalized-message), where category is the category field in the reviewer_findings.v1alpha1 schema and normalized-message is derived from the message field by: (1) lowercasing the entire string, (2) trimming leading and trailing whitespace, and (3) collapsing any run of internal whitespace characters to a single space. Two findings from different rounds are the same open blocker when their canonical keys match. This key is used only for fixer-no-progress streak detection; it is not a persistent finding ID and does not affect reviewer-output capture or parse.
...
- mem_023 [fresh] score=61 — Plan-DAG slice 3: the plan immune system (entry) — static non-vacuous validat...
- mem_024 [fresh] score=58 — Plan-DAG slice 4b: the minimal topological execute loop — run a contract's pl...
- mem_016 [fresh] score=58 — Port the code-review loop (internal/app/review_loop.go) onto the existing int...
- mem_005 [fresh] score=57 — Make the CLI announce legal moves so an agent never guesses the pipeline stat...
