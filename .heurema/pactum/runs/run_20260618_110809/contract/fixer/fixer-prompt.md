...
Current contract version: 3dbd2373d4c12ac328455452308767ce83eae83bb70873b1f305ebb0bb875d1f
...
1. [opus/validation-soundness] Validation command `make check` is contradictory with the scoped deliverable and will fail on a correct in-scope implementation. `make check` runs the `deadcode` target (Makefile:40-46 -> `go tool deadcode ./...`, no `-test` flag), which per the Makefile's own comment fails the gate on 'production code reachable only from tests.' Slice 4a's deliverables are exactly that: three new package-level functions (context-pack builder, frozen-validation runner, baseline-red checker) whose only callers in 4a are unit tests, because the contract defers all production wiring to 4b (Scope out: 'Do not add ... per-node loop.Run wiring'; 'Do not change execute run's current single-shot behavior') and states 'no public CLI for running an individual node is required in this slice.' Result: `deadcode` flags the new helpers as unreachable and `make check` fails on a faithful in-scope build, making the contract ungatable. Resolve by requiring a thin reachable entry point in 4a (e.g., a `pactum plan context <task_id>` diagnostic command that invokes the primitives) so deadcode stays green, or by explicitly exempting `deadcode`/documenting that the primitives must be reachable from main.
   Evidence: Validation command: `make check` (Makefile:65 `check: test vet deadcode`; Makefile:40-46 deadcode comment: 'unused package-level functions, including production code reachable only from tests. Any finding fails the gate'). Contract Scope out: 'Do not add the topological scheduler, graph-drain loop, per-node loop.Run wiring ...' and 'Do not change execute run's current single-shot behavior'. Contract Assumptions: 'The 4a primitives may remain internal Go helpers with unit-test coverage; no public CLI for running an individual node is required in this slice.'
2. [codex-xhigh/completeness] Baseline-red classification does not specify behavior for tasks with multiple validation commands. The contract says to run a task's validation[] but requires a singular result with one status, shape, command, and recommendation, leaving aggregation order and mixed outcomes undefined.
   Evidence: “run a task's validation against the UNCHANGED working tree” and “Return a structured baseline result {status: red|green, shape: test|other, command, recommendation: proceed|block|signal}”
3. [codex-xhigh/assumptions-surfaced] The contract assumes symbol selectors resolve uniquely and deterministically, but does not state behavior for no match, multiple matches, stale index data, or non-deterministic search ordering.
   Evidence: “for a symbol selector, resolve the symbol to a path:start-end via the existing search/code-index (the same mechanism `pactum search --symbol` uses)”
4. [codex-xhigh/assumptions-surfaced] The contract assumes path+line selectors are valid and readable, but does not define behavior for missing files, invalid line ranges, binary/unreadable files, or ranges past EOF.
   Evidence: “For a path+lines selector, read the repo-relative file and emit the line-range slice”
...
```json
{
  ...
  "base_version": "3dbd2373d4c12ac328455452308767ce83eae83bb70873b1f305ebb0bb875d1f",
  "contract": {
