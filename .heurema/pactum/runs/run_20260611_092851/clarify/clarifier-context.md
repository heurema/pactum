...
- Coverage by dimension (a zero-question dimension is unprobed, not necessarily settled):
  ...
  - edge_case: total 2, answered 0, open 2, blocking open 0
  - assumption: total 1, answered 1, open 0, blocking open 0
- Questions:
  - q_001 blocking=true status=answered: When the contract says the CLI must never prompt, is the scope limited to Pactum's own interactive confirmation prompts and `--yes` guards, leaving underlying agent transport behavior unchanged?
    kind: assumption
  ...
  - q_003 blocking=false status=open: Should automatic loop-created decisions get a `--by` principal field, for example clarify-loop auto answers and review-loop duplicate proposal decisions?
    ...
    recommended answer (confidence high): Do not add a principal to automatic loop decisions. Keep their existing `source` values as the actor/provenance signal, and only record `decided_by`/`accepted_by` for explicit CLI decision verbs that accept `--by`.
    depends on: q_002
    blocked: waiting on unanswered prerequisites
  - q_004 blocking=false status=open: How should empty, whitespace-only, or path-like `--by` values be normalized across all decision verbs?
    kind: edge_case
    ...
    recommended answer (confidence high): Trim whitespace before persistence; if the trimmed value is empty, record `manual`; sanitize any repo-root absolute path text the same way memory acceptance does; otherwise preserve the non-empty principal string.
    depends on: q_002
    blocked: waiting on unanswered prerequisites
  - q_005 blocking=false status=open: After deleting `gate run --allow-commands`, should the existing gate report field `validation.commands_allowed` remain in the JSON schema, and what value should it have when there are zero validation commands?
    kind: acceptance
    ...
    recommended answer (confidence medium): Keep `validation.commands_allowed` for now and set it to `true` on every successful `gate run`; with zero validation commands, emit `commands_allowed: true` and an empty `commands` array.
  - q_006 blocking=false status=open: Should historical docs such as backlog and dogfood records be rewritten to remove old `--yes` and `--allow-commands` examples, or only current guidance?
    kind: scope
    ...
    recommended answer (confidence medium): Update all current user/agent guidance, helper text, generated hints, tests, scripts, README, AGENTS, docs/agents, docs/flow, docs/agent-skill, and bundled skill references. Leave clearly historical backlog/dogfood transcripts intact only if they are not presented as current instructions.
  - q_007 blocking=true status=answered: When the contract says `--by` should be uniform across all decision verbs, should that mean only the explicitly named principal-bearing commands (`contract approve`, `review approve`, `memory accept`, `contract accept`, `clarify answer`, `review proposal accept`, and `review proposal reject`), or should it also include other mutating decision-like commands such as `clarify add`, `contract revise`, `review finding add`, `review finding resolve`, `review prepare`, `memory propose`, or `memory refresh`?
    kind: terminology
