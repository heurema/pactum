...
- Validation command results:
  - command_001: go test ./internal/app -run TestGitGuard (exit 0, timed out: false, result: gate/validation/command_001/result.json)
...
- Existing resolutions:
  - none
- Proposal summary: pending=2 accepted=0 rejected=0
- Existing proposals:
  - p_001 severity=medium category=validation blocking=true status=pending source=reviewer_attempt attempt=reviewer_attempt_003: The linked-worktree test only checks that git status contains agent.txt, so it would also pass if the restored work were staged. The contract requires the test to demonstrate that the committed work is left present but unstaged.
    location: internal/app/git_guard_test.go:684
  - p_002 severity=medium category=quality blocking=true status=pending source=reviewer_attempt attempt=reviewer_attempt_004: The linked-worktree restore test does not prove the agent work is left unstaged after restore.
    location: internal/app/git_guard_test.go:684
